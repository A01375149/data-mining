...
Escribe la fecha actual (**NOTE-01**): 03 de junio del 2021
...
```{r}
NOMBRE_COMPLETO = "Olga Paulina Corona Trejo"
DIGITOS_MATRICULA = 01375149
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): No hay relación entre el producto de pañales y algún otro artículo 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Representa cada una de las sesiones del dataset y su objetivo consiste en distinguir el número de sesiones que los usuarios tiene en spotify y así poder hacer recomendaciones 
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Representar cada una de las canciones, además de distinguir cuáles son las canciones que escuchan varios usuarios y con base en sus preferencias hacer recomendaciones  
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que representan el comportamiento que los usuarios tienen al escuchar música en spotify. Se pueden hacer asociaciones si se entiende la relación entre los gustos y las canciones a través de las pausas, canciones saltadas, etc.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce - 872
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): Itemset 20 con 1576
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen un promedio de 14.64 sesiones 
...
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que se pierda presición en el análisis puesto que se tendrá un mayor número de reglas de asociación que pueden no ser tan certeras. 
...
- Provee el enlace al video aquí: **NOTE-30** Enlace al video: https://drive.google.com/drive/folders/19NAfQW1YLc53MiSn08Upo8ucdc1Xa0aV?usp=sharing
